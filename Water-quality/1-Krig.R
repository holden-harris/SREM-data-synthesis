...
###########################################################################################
##   Depth base map
depth.out = raster("./0-Data/habitats/processed/crm-salt-marsh-corrected/crm 60x50 18s 485m - saltmarsh corrected.gri") ## High res depth map with salt marsh corrections  
depth.out; plot(depth.out, colNA='black')

###########################################################################################
##  WQ data
wq.base = read.csv("./0-Data/water-quality/processed/Spatial-temp_Phys-Flow_xMonth.csv")
...
maxval = 40
modtype = "KRG"
dir.out = "./1-Prepare-Ecospace-Inputs/water-quality/out/KRG/"
